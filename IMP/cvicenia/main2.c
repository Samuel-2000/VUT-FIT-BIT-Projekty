 ...
 * -------------------------------------------------------------------------- */
void beep(freq) {
	int len = 200 * 200 / freq;
    for (uint32_t q=0; q<len; q++) {
        GPIOB_PDOR |=  PIEZZO_MASK; delay(freq);
        GPIOB_PDOR &= ~PIEZZO_MASK; delay(freq);
    }
}
 ...
 * pipnul piezo-bzucakem - funkce beep(200)(). Zajistete korektni osetreni zakmitu *
...
void PORTB_IRQHandler(void) {
     ...
     * - dale provedte odezvu na stisk tlacitka (volani funkci flash() a beep(200)())
    ...
    if (PORTB->ISFR & BUTTON_UP_MASK) {
        ...
        PORTB->PCR[3] = PORTB->PCR[3] | PORT_PCR_ISF(1);  /* Vynuluje priznak */
    } else if (PORTB->ISFR & BUTTON_CENT_MASK) {
    	if (GPIOB_PDIR) {
    		beep(1260);
    		flash();
    	}
    	PORTB->PCR[4] = PORTB->PCR[4] | PORT_PCR_ISF(1);
    } else if (PORTB->ISFR & BUTTON_LEFT_MASK) {
    	if (GPIOB_PDIR) {
    		beep(1414);
    		flash();
    	}
		PORTB->PCR[5] = PORTB->PCR[5] | PORT_PCR_ISF(1);
    } else if (PORTB->ISFR & BUTTON_DOWN_MASK) { /* tlacitko DOLU (Down) */
    	if (GPIOB_PDIR) {
    		beep(1500);
    		flash();
    	}
		PORTB->PCR[6] = PORTB->PCR[6] | PORT_PCR_ISF(1);
    } else if (PORTB->ISFR & BUTTON_RIGHT_MASK) { /* tlacitko DOPRAVA (Right) */
    	if (GPIOB_PDIR) {
    		beep(1120);
    		flash();
    	}
...
void init_hardware(void) {
    ...
    // mohli tyto komponenty programove ridit - vizte funkce beep(200)(), flash()
