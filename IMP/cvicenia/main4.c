...
/*                                                                            */
/* Reseni vytvoril(a) a odevzdava: (Samuel Kuchta, xkucht11)                  */
/*                                                                            */
/******************************************************************************/
...
void ADC0_Init(void) {
    NVIC_EnableIRQ(ADC0_IRQn);  // zpracovani namerene hodnoty v ramci obsluhy preruseni, Nezapomente povolit preruseni v systemu NVIC (inspirace viz 2. cviceni

    ADC0_CFG1 = ADC_CFG1_MODE(0) | ADC_CFG1_ADIV(3) | ADC_CFG1_ADICLK(1);  // zdroj hodinoveho signalu: (Bus clock)/2, clock divider = 8
    ADC0_SC3 = ADC_SC3_AVGS(3) | ADC_SC3_ADCO(1) | ADC_SC3_AVGE(1);  //  vysledek bude hardwarove prumerovan z 32 vzorku
    ADC0_SC1A = ADC_SC1_AIEN(1) | ADC_SC1_ADCH(6);  //  rezim prevodu: kontinualni konverze, 8-bit, ADC0_SC1A JE NUTNO NASTAVIT JAKO POSLEDNI
}


void ADC0_IRQHandler(void) {
    if (ADC0_SC1A & ADC_SC1_COCO_MASK) {
        sprintf(result, "%.2f\0", ADC0_RA*3.3 / 255.0);
    }
}
/******************************************************************************/
